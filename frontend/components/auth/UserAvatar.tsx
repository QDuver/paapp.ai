...
interface UserAvatarProps {
  user: { email: string | null; uid: string; photoURL?: string | null; displayName?: string | null } | null;
  size?: number;
  onPress?: () => void;
}
...
const UserAvatar = ({ user, size = 36, onPress }: UserAvatarProps) => {
  ...
  const content = user?.photoURL ? (
    ...
    <View style={{ width: size, height: size, borderRadius: size / 2, backgroundColor: '#444', justifyContent: 'center', alignItems: 'center' }}>
